...
export async function POST(request: NextRequest) {
  ...
  if (!canAccess) {
    return jsonError("Authorize this wallet before pausing Spend&Save.", 401);
  }

  try {
    const config = await setSpendSaveEnabled(ownerWallet, "pause");
    return NextResponse.json({ config });
  } catch (error) {
    const message =
      error instanceof Error
        ? error.message
        : readSavingsSupabaseError(null, "Could not pause Spend&Save.");
